...
async def get_user(user_id: int = Path(..., gt=0, description="Unique user identifier")):
...
async def delete_user(user_id: int = Path(..., gt=0, description="Unique user identifier")):
    """Delete a user by ID"""
    if user_id not in users_db:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
